...
import { generateTopContributions } from '@/lib/actions/ai-actions'
...
export async function GET(
  ...
  try {
    ...
    if (!stats) {
      return new NextResponse(loadingSvg.replace('Loading GitHub Contributions...', 'GitHub Profile Not Found'), {
        headers: {
    ...
    // Get top contributions from AI
    const contributions = await generateTopContributions(
    ...
    // Take top 3 contributions for the widget
    ...
    <svg width="800" height="260" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect width="800" height="260" fill="#1a1b1e"/>
      
      <!-- Title -->
      <text x="35" y="45" class="title">Top Contributions</text>
      
      <!-- Contribution Cards -->
      ${topThree.map((contribution, index) => {
        // Split impact text into words and create lines
        const words = contribution.impact.split(' ');
        let lines = [];
        ...
        if (lines.length > maxLines) {
          ...
          const lastLine = lines[maxLines - 2];
          if (lastLine.length > maxLineLength - 3) {
            lines[maxLines - 2] = lastLine.slice(0, maxLineLength - 3) + '...';
          } else {
            lines[maxLines - 2] = lastLine + '...';
          }
        }

        // Truncate repo name if too long
        const maxRepoLength = 25;
        const truncatedRepo = contribution.repo.length > maxRepoLength 
          ? contribution.repo.slice(0, maxRepoLength - 3) + '...'
          : contribution.repo;

        return `
        <g transform="translate(${35 + index * 245}, 75)">
          <rect class="card" width="230" height="160" rx="8"/>
          ${lines.map((line, i) => 
            `<text x="20" y="${30 + i * 20}" class="contribution">${line}</text>`
          ).join('')}
          <g transform="translate(20, 130)">
            <svg width="14" height="14" viewBox="0 0 16 16">
              <path fill="#888" d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"/>
            </svg>
            <text x="24" y="11" class="repo">${truncatedRepo}</text>
          </g>
        </g>
      `}).join('')}
        ...
        <text
         x="720" y="20" fill="#777" font-size="10px">statsforgit.com</text>
        </svg>`.trim()
    ...
    console.error('Error generating GitHub contributions widget:', error)
    return new NextResponse(loadingSvg.replace('Loading GitHub Contributions...', 'Error Loading Contributions'), {
      headers: {
        'Content-Type': 'image/svg+xml',
        'Cache-Control': 'max-age=0, s-maxage=3600',
        'Access-Control-Allow-Origin': '*',
