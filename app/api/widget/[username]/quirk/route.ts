...
import { generateAchillesHeel } from '@/lib/actions/ai-actions'
...
export async function GET(
  ...
  try {
    ...
    if (!stats) {
      return new NextResponse(loadingSvg.replace('Loading Developer Quirk...', 'GitHub Profile Not Found'), {
        headers: {
    ...
    // Get achilles heel/quirk from AI
    const quirk = await generateAchillesHeel(
      stats,
    ...
    const gradientId = 'quirkGradient'
    const svg = `<?xml version="1.0" encoding="UTF-8"?>
    <svg width="800" height="200" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="${gradientId}" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stop-color="${quirk.color.from}" stop-opacity="0.1"/>
          <stop offset="100%" stop-color="${quirk.color.to}" stop-opacity="0.1"/>
        </linearGradient>
      </defs>
      ...
      <style>
        ...
        .quirk-title { fill: white; font-size: 24px; font-weight: 500; }
        .description { fill: #888; font-size: 16px; }
        .tip { fill: #10B981; font-size: 14px; }
      </style>
      ...
      <!-- Title -->
      <text x="40" y="60" class="title">Developer Quirk</text>
      
      <!-- Quirk Info -->
      <text x="40" y="100" class="quirk-title">
        ${quirk.icon} ${quirk.title}
      </text>
      <text x="40" y="130" class="description">${quirk.description}</text>
      <text x="40" y="160" class="tip">💡 Quick Tip: ${quirk.quickTip}</text>
    ...
    console.error('Error generating GitHub quirk widget:', error)
    return new NextResponse(loadingSvg.replace('Loading Developer Quirk...', 'Error Loading Quirk'), {
      headers: {
        'Content-Type': 'image/svg+xml',
        'Cache-Control': 'max-age=0, s-maxage=3600',
        'Access-Control-Allow-Origin': '*',
